...
group_img = load(fullfile(pathtodata,'accuracyGroup_IMAGERY_Single_stim_N=21_high250.mat'));
group_per = load(fullfile(pathtodata,'accuracyGroup_PERCEPTION_Single_stim_N=21_high250.mat'));
group_cross = load(fullfile(pathtodata,'accuracyGroup_CROSS_Single_stim_N=21_high250.mat'));
...
for k = 1: niter
    ...
    for iSub = 1: length(subList)
        %load file
        perm_img = load(fullfile(outputpath,sprintf('sub%02d_IMAGERY_LDA_permuted_rois.mat', subList(iSub))));
        perm_per = load(fullfile(outputpath,sprintf('sub%02d_PERCEPTION_LDA_permuted_rois.mat', subList(iSub))));
        perm_cross = load(fullfile(outputpath,sprintf('sub%02d_CROSS_LDA_permuted_rois.mat', subList(iSub))));
        a_per=[];a_img=[];a_cross=[];
    ...
    permuted_group_ds{k}.sa.chunks=(1:n_samples)';     % all subjects are independent
    permuted_group_ds{k}.sa.targets=ones(n_samples,1); % one-sample t-test
...
ds_z_permuted_maxsum=cosmo_montecarlo_cluster_stat(ds_all_real,nbrhood,'niter',10000, 'h0_mean', chance,'progress',10, 'null', permuted_group_ds,'cluster_stat', 'maxsum', 'p_uncorrected', 0.001);
% % SAVE ACCURACY FILE 
save([outputpath filesep 'ds_z_permuted'],'ds_z_permuted');
%Transform values in p-values
ds_p_permuted = normcdf(ds_z_permuted.samples)
% Save p_values 
save([outputpath filesep 'ds_p_permuted'],'ds_p_permuted');
