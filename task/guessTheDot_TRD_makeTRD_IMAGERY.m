function guessTheDot_TRD_makeTRD_IMAGERY(trdName, iSub, side, plotDesign)
%
...
%
% Example call
% - guessTheDot_TRD_makeTRD_IMAGERY('guessTheDot', 99)
% NOTE: you can also simply use guessTheDot_TRD_makeTRD_IMAGERY for testing
%
...
Info.crossDurSec        = 1;              % presented only at the beginning of a block (In sec)
Info.preStim            = 1;              % event1 (In sec)
Info.cueDurSec          = 0.5; %1/60;     % event2 (In sec)
Info.imDurSec           = 7;              % event3 (In sec)
Info.respDurSec         = 2;              % event4 (In sec)
Cfg.WaitSecs            = 0.5;
Info.trialDurSec        = Info.stimDurSec+Info.maskDurSec+Info.preStim+Info.cueDurSec+Info.imDurSec+Info.respDurSec;
...
%             Info_block.expStructure=Info_block.expStructure(Info_block.expStructure(:,1)==iBlock-1,:);
            ...
            if writeSeparatedBlocks
                writeTRD(TrialDefinitions{iBlock}, Info, fullfile('.','trd', sprintf('%s_S%02d_R%02d_IMAGERY.trd', trdName, iSub, iBlock))) %#ok<UNRCH>
            end
        ...
        writeTRD(TrialDefinitionsRUNBlank, Info, fullfile('.','trd', sprintf('%s_S%02d_R%02d_IMAGERY.trd', trdName, iSub, iRun))) %#ok<UNRCH>
    end
...
for iTrial = 1:size(Info.expStructure,1)
    ...
    thisTrial.page      = [Info.cueSTD(thisTrial.userDefined(4)+1) Info.placeHold dotImg Info.placeHold];
    thisTrial.durations = [Info.cueDurSec Info.imDurSec Info.respDurSec (Info.preStim-Cfg.WaitSecs)]*Cfg.Screen.Resolution.hz;
...
nTrials = length(TrialDefinitionsRUNBlank);
% Maybe pass this ITI vector into the function, too
jitvec = 0;%1:0.5:2;

% Define the first onset, since it is currently NaN;
TrialDefinitionsRUNBlank(1).tOnset = 0;
for iTrial = 2:nTrials
    %CALCULATE ONSET AND DURATION OF PREVIOUS TRIAL WITH WAITSECS
...
firstTrial.page             = [Info.pageEv1 Info.pageMask Info.placeHold]; %[Info.placeHold Info.pageEv1 Info.maskSTD];
...
blankTrial.page             = [Info.placeHold Info.pageEv1 Info.maskSTD];
blankTrial.durations        = [1 Info.stimDurSec Info.maskDurSec]*Cfg.Screen.Resolution.hz;
