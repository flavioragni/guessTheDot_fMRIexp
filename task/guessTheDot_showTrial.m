...
PsychPortAudio('FillBuffer', Cfg.Sound.playbackHandle(Cfg.Sound.iDevice), Cfg.Sound.wavedata{atrial.userDefined(4)+1});
...
for i = 1:atrial.startRTonPage-1
    if (i > atrial.nPages)
        break;
    elseif atrial.code>0
        if i==1
            ASF_playSound(Cfg, 0, Cfg.Sound.iDevice);
        end
    end
